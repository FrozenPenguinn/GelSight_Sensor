...
plt.title("gradient-intensity map (Blue)")
plt.imshow(blue, origin='lower', extent=[-3, 3, -3, 3])
plt.colorbar()
plt.show()
'''
...
for x in range(0, width):
    for y in range(0, height):
        if (pow(x - center_w, 2) + pow(y - center_h, 2) <  pow(radius - 5, 2)):
            error_map[x][y] = pow(grad[x][y][0] - gradmap[x][y][0], 2) + pow(grad[x][y][1] - gradmap[x][y][1], 2)
            count += 1
...
'''
'''
# height reconstruction by local iteration method
...
x, y = np.meshgrid(x, y)
ax.plot_surface(x, y, zmap, rstride = 10, cstride = 10, cmap = plt.get_cmap('rainbow'))
